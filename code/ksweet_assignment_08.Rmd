...
indig<-left_join(sw.indig,extracts.df)
...
indig.county.az <- indig.county.az %>% 
  ...
  select(GEOID, meaninc, meang)

indig.county.az<-left_join(indig.county.az,census, by="GEOID")
...
extracts.df <- map(1:nrow(regional.pas.sf), function(x){
  data.frame(Loc_Nm = st_drop_geometry(regional.pas.sf[x, "Loc_Nm"]),
             land.val = mean(ras[[x]][[]], na.rm = TRUE),
             Value = mean(ras[[x]][[2]], na.rm = TRUE))
             }) %>% do.call(rbind, .)
...
tm_shape(regional.pas.sf) +
  ...
  tm_shape(indig) +
  tm_polygons(col = "estimate", palette=viridis(n=10)) +
  tm_legend(outside = TRUE)+
  tm_shape(state)+
  tm_borders()
  ...
  tm_shape(indig) +
  tm_polygons(col = "medianincome", palette=viridis(n=10)) +
  tm_legend(outside = TRUE)+
  tm_shape(state)+
  tm_borders()
...
ggmap(bg) +
   geom_sf(data = indig.county.az, mapping = aes(fill =  meaninc), inherit.aes = FALSE) +
  geom_sf(data=az, fill=NA,color="black", inherit.aes = FALSE) +
  coord_sf(crs = st_crs(albers))

ggmap(bg) +
  geom_sf(data = indig.county.az, mapping = aes(fill = Value, 
                                           alpha = (Value - max(Value, na.rm=TRUE))/(max(Value, na.rm=TRUE)-min(Value, na.rm = TRUE))), inherit.aes = FALSE) +
  geom_sf(data=az, fill=NA,color="black", inherit.aes = FALSE) +
...
It looks like on average there's a lot of mammal species richness in reservation land of arizona when cmpared to the rest of the state. Another interestig thing was when I looked at the data in there.
...
No, not necessarly. For example the tmap would 
